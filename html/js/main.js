...
//	alert(xmlDoc);
		...
		for(var i = 0; i < LenCat; i++)	{
			timestamp = $(xmlDoc2).find("timestamp").text();
//			alert("1 "+xmlDoc);
//			poiName = xmlDoc.getElementsByTagName("Poi")[i].attributes.getNamedItem("Name").nodeValue;
			poiName = xmlDoc2.getElementsByTagName("pois")[0].getElementsByTagName("Poi")[i].getElementsByTagName("Name")[0].textContent;
			poiDescr = xmlDoc2.getElementsByTagName("Poi")[i].getElementsByTagName("Description")[0].textContent;
			poiLong = xmlDoc2.getElementsByTagName("Poi")[i].getElementsByTagName("Longitude")[0].textContent;
			poiLat = xmlDoc2.getElementsByTagName("Poi")[i].getElementsByTagName("Latitude")[0].textContent;
...
//			alert(poiName+" "+poiDescr+" "+poiLong+" "+poiLat+" "+poiCat+" "+subCatId+" time: "+timestamp);
			tx.executeSql('INSERT INTO POIEN (name, descr, category, subcategory, long, lat, timestamp) VALUES (?,?,?,?,?,?,?)'
					,[poiName, poiDescr, poiCat, subCatId, poiLong, poiLat, timestamp], successCB, error2CB);
		}
	});

	db.transaction(function (tx) {
		tx.executeSql('SELECT * FROM POI', [], function (tx, results) {
			var len = results.rows.length, i;
			for (i = 0; i < len; i++){
//				alert('poiDb['+i+']: '+results.rows.item(i).name+", "+results.rows.item(i).descr+", "
//						+results.rows.item(i).category+", "+results.rows.item(i).subcategory+", "
//						+results.rows.item(i).long+", "+results.rows.item(i).lat);
			}
//			alert(dbtext);
		}, errorCB);
	});
...
//				category.push(results.rows.item(i).id);
			}
//			alert(dbtext);
			len = nameCat.length;
//			alert("Catlen "+len);
	...
	db.transaction(function (tx) {
		tx.executeSql('SELECT * FROM SUBCATEGORIESEN', [], function (tx, results) {
			...
			for (var j=0; j<checked.length; j++){
				alert("("+checked[j]+")");
				for (var i = 0; i < len; i++){
					alert("_"+results.rows.item(i).name+"_");
					if (checked[j] == results.rows.item(i).name){
